...
func (p *Parser) Parse(lines []string) ([]Data, error) {
	...
	for _, parseResult := range parseResultSlice {
		...
		log.Infof("D! parse result(%v)", parseResult.Data)
		se.AddSuccess()
...
func (p *Parser) parse(line string) (Data, error) {
	...
	for k, v := range values {
		...
		case LONG:
			...
			if err != nil {
				log.Warningf("E! Error parsing %s to long: %s, ignore this field...", v, err)
			} else {
				data[k] = iv
			}
		case FLOAT:
			...
			if err != nil {
				log.Warningf("E! Error parsing %s to float: %s, ignore this field...", v, err)
			} else {
				data[k] = fv
			}
		case DATE:
			...
			if err == nil {
				...
				log.Warningf("E! Error parsing %s to time layout [%s]: %s, ignore this field...", v, t, err)
			}
