...
import (
	...
	"github.com/longxiucai/logkit/queue"
	. "github.com/longxiucai/logkit/utils/models"
	"github.com/longxiucai/logkit/utils/reqid"
)
...
func (ft *FtSender) Close() error {
	...
	log.Warningf("Runner[%v] wait for Sender[%v] to completely exit", ft.runnerName, ft.Name())
	// 等待错误恢复流程退出
	<-ft.exitChan
	// 等待正常发送流程退出
	...
	log.Warningf("Runner[%v] Sender[%v] has been completely exited", ft.runnerName, ft.Name())
...
func (ft *FtSender) handleSendError(err error, datas []Data) (retDatasContext []*datasContext) {
	...
	log.Infof("Runner[%v] Sender[%v] failed datas [[%v]]", ft.runnerName, ft.innerSender.Name(), failCtx.Datas)
	if binaryUnpack {
		...
		if len(failCtx.Datas) == 1 {
			...
			if int64(len(string(dataBytes))) < DefaultMaxBatchSize {
				...
				if _, ok := failCtxData[KeyPandoraStash]; !ok {
					...
					if err != nil {
						log.Warningf("Runner[%v] marshal data to string error %v", ft.runnerName, err)
					} else {
						data := make([]Data, 1)
						data[0] = Data{
