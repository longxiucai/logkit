...
func (s *Sender) Send(datas []Data) error {
	...
	for _, d := range datas {
		p, err := s.makePoint(d)
		if err != nil {
			log.Warningf("Runner[%s] %s make point format err : %v", s.runnerName, s.Name(), err)
			continue
		}
		ps = append(ps, p)
	}
	...
	if err != nil {
		if s.ignoreBeyondRetention && isBeyondRetentionErr(err) {
			...
			log.Warningf("Runner[%s] %s data is out of retention and ignore_beyond_retention is true, some of ignored data %s it", s.runnerName, s.Name(), err, partialData)
			return nil
		}
...
func (s *Sender) makePoint(d Data) (p Point, err error) {
	...
	for k, t := range s.tags {
		v, exist := d[k]
		if !exist {
			log.Infof("Runner[%s] %s tag %s not exist", s.runnerName, s.Name(), t)
			continue
		}
	...
	for k, f := range s.fields {
		v, exist := d[k]
		if !exist {
			log.Infof("Runner[%s] %s field %s not exist", s.runnerName, s.Name(), f)
			continue
		}
		fields[f] = v
	}
