...
func NewSingleFile(meta *Meta, path, whence string, errDirectReturn bool) (sf *SingleFile, err error) {
	...
	for {
		...
		if err != nil || pfi == nil {
			...
			log.Warningf("Runner[%v] %s - utils.GetRealPath failed, err:%v", meta.RunnerName, path, err)
		...
		if !pfi.Mode().IsRegular() {
			...
			log.Warningf("Runner[%v] %s - file failed, err: file is not regular", meta.RunnerName, path)
			time.Sleep(time.Minute)
			continue
		}
		f, err = os.Open(path)
		if err != nil {
			...
			log.Warningf("Runner[%v] %s - open file err:%v", meta.RunnerName, path, err)
			time.Sleep(time.Minute)
			continue
		}
		break
	...
	if err != nil {
		if os.IsNotExist(err) {
			log.Infof("Runner[%v] %v -meta data is corrupted err:%v, omit meta data", meta.RunnerName, meta.MetaFile(), err)
		} else {
			log.Warningf("Runner[%v] %v -meta data is corrupted err:%v, omit meta data", meta.RunnerName, meta.MetaFile(), err)
		}
		omitMeta = true
	}
	if metafile != originpath {
		log.Warningf("Runner[%v] %v -meta file <%v> is not current file <%v>， omit meta data", meta.RunnerName, meta.MetaFile(), metafile, originpath)
		omitMeta = true
	}
	...
	if omitMeta {
		...
		log.Infof("Runner[%v] %v restore meta success", sf.meta.RunnerName, sf.Name())
	}
	sf.offset = offset
	st, err := f.Stat()
	if err != nil {
...
func (sf *SingleFile) statFile(path string) (pfi os.FileInfo, err error) {
	...
	for {
		...
		path, pfi, err = GetRealPath(path)
		if err != nil || pfi == nil {
			log.Warningf("Runner[%v] %s - utils.GetRealPath failed, err:%v", sf.meta.RunnerName, path, err)
			time.Sleep(time.Minute)
			continue
		}
		break
...
func (sf *SingleFile) SyncMeta() error {
	...
	log.Infof("Runner[%v] %v Sync file success: %v", sf.meta.RunnerName, sf.Name(), sf.offset)
