...
func getStartFile(path, whence string, meta *Meta, sf *SeqFile) (f *os.File, dir, currFile string, offset int64, err error) {
	...
	if err != nil {
		...
		log.Infof("%v restore meta success", dir)
	}
...
func NewSeqFile(meta *Meta, path string, ignoreHidden, newFileNewLine bool, suffixes []string, validFileRegex, whence string) (sf *SeqFile, err error) {
	...
	if err != nil {
		...
		log.Warningf("Runner[%v] NewSeqFile reader getStartFile from dir %v error %v, will find during read...", sf.meta.RunnerName, path, err)
		err = nil
		dir = path
	}
	if f != nil {
...
func (sf *SeqFile) getIgnoreCondition() func(os.FileInfo) bool {
	return func(fi os.FileInfo) bool {
		...
		if !match {
			log.Infof(" when read dir %s, get no valid file in pattern %v", sf.dir, sf.validFilePattern)
		}
		return match
	}
}
...
func (sf *SeqFile) reopenForESTALE() error {
	log.Warningf("reopening stale NFS file handle for %q", sf.currFile)
...
func (sf *SeqFile) Read(p []byte) (n int, err error) {
	...
	for n < len(p) {
		...
		if sf.f == nil {
			...
			if err != nil {
				if !os.IsNotExist(err) {
					log.Warningf("Runner[%v] %v new open error %v", sf.meta.RunnerName, sf.dir, err)
				}
		...
		if err != nil {
			...
			if os.IsNotExist(err1) {
				...
				log.Infof("Runner[%v] %s - nextFile: %v", sf.meta.RunnerName, sf.dir, err1)
				time.Sleep(WaitNoSuchFile)
				nextFileRetry++
				continue
			}
			if err1 != nil {
				log.Infof("Runner[%v] %s - nextFile(file exist) but: %v", sf.meta.RunnerName, sf.dir, err1)
				return n, err1
			}
			if fi != nil {
...
func (sf *SeqFile) getNextFileCondition() (condition func(os.FileInfo) bool, err error) {
	...
	if err != nil {
		...
		err = nil
		// 当前读取的文件已经被删除
		log.Infof("Runner[%v] stat current file [%v] error %v, start to find the oldest file", sf.meta.RunnerName, sf.currFile, err)
		return
	}
...
func (sf *SeqFile) nextFile() (fi os.FileInfo, err error) {
	...
	if err != nil {
		log.Infof("Runner[%v] getMinFile error %v", sf.meta.RunnerName, err)
		return nil, err
	}
	if sf.isNewFile(fi, filepath.Join(sf.dir, fi.Name())) {
		return fi, nil
	} else {
		log.Warningf("Runner[%v] %v is not new file", sf.meta.RunnerName, fi.Name())
	}
	return nil, nil
}
...
func (sf *SeqFile) isNewFile(newFileInfo os.FileInfo, filePath string) bool {
	...
	if newInode != sf.inode {
		log.Infof("Runner[%v] %s - newInode: %d, l.inode: %d", sf.meta.RunnerName, sf.dir, newInode, sf.inode)
		return true
	}
	if newFsize < sf.offset {
		log.Infof("Runner[%v] %s - newFsize: %d, l.offset: %d", sf.meta.RunnerName, sf.dir, newFsize, sf.offset)
		return true
	}
	fname := filepath.Base(sf.currFile)
	if newName != fname {
		log.Infof("Runner[%v] %s - newName: %d, l.fname: %d", sf.meta.RunnerName, sf.dir, newName, fname)
		return true
	}
	return false
}
...
func (sf *SeqFile) open(fi os.FileInfo) (err error) {
	...
	if err != nil && err != syscall.EINVAL {
		log.Warningf("Runner[%v] %s - %s f.Close: %v", sf.meta.RunnerName, sf.dir, sf.currFile)
		return
	}
	...
	f, err := os.Open(sf.currFile)
	if err != nil {
		log.Warningf("Runner[%v] os.Open %s: %v", sf.meta.RunnerName, fname, err)
		return err
	}
	sf.f = f
	//开新的之前关掉老的
...
func (sf *SeqFile) SyncMeta() (err error) {
	...
	if sf.lastSyncOffset == sf.offset && sf.lastSyncPath == sf.currFile {
		log.Infof("Runner[%v] %v was just syncd %v %v ignore it...", sf.meta.RunnerName, sf.Name(), sf.lastSyncPath, sf.lastSyncOffset)
		return nil
	}
