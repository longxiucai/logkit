...
func NewReader(meta *reader.Meta, conf conf.MapConf) (reader.Reader, error) {
	...
	if certfile != "" {
		log.Warningf("Runner[%v] MongoDB reader does not support certfile Now", meta.RunnerName)
		//TODO mongo鉴权暂时不支持
	}
	r := &Reader{
		meta:              meta,
...
func (r *Reader) run() {
	...
	log.Warningf("Runner[%v] %q task execution failed and gave up after 10 tries", r.meta.RunnerName, r.Name())
}
...
func (r *Reader) exec() (err error) {
	...
	for iter.Next(&result) {
		if r.isStopping() || r.hasStopped() {
			log.Warningf("Runner[%v] %q daemon has stopped, iteration is interrupted", r.meta.RunnerName, r.Name())
			return nil
		}
