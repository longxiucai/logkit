...
func (ar *ActiveReader) Run() {
	...
	for {
		if atomic.LoadInt32(&ar.status) == reader.StatusStopped || atomic.LoadInt32(&ar.status) == reader.StatusStopping {
			...
			log.Warningf("Runner[%v] ActiveReader %s was stopped", ar.runnerName, ar.originpath)
			return
		}
		if ar.readcache == "" {
			...
			if err != nil && err != io.EOF && err != os.ErrClosed {
				log.Warningf("Runner[%v] ActiveReader %s read error: %v", ar.runnerName, ar.originpath, err)
			...
			if ar.readcache == "" {
				...
				if err == io.EOF {
					...
					log.Infof("Runner[%v] %v meet EOF, ActiveReader was inactive now, sleep 5 seconds", ar.runnerName, ar.originpath)
		...
		log.Infof("Runner[%v] %v >>>>>>readcache <%v> linecache <%v>", ar.runnerName, ar.originpath, ar.readcache, string(ar.br.FormMutiLine()))
		repeat := 0
		for {
			if ar.readcache == "" {
				break
			...
			//做这一层结构为了快速结束
			if atomic.LoadInt32(&ar.status) == reader.StatusStopped || atomic.LoadInt32(&ar.status) == reader.StatusStopping {
				log.Infof("Runner[%v] %v ActiveReader was stopped when waiting to send data", ar.runnerName, ar.originpath)
				atomic.CompareAndSwapInt32(&ar.status, reader.StatusStopping, reader.StatusStopped)
				return
			}
			select {
...
func (ar *ActiveReader) Close() error {
	defer log.Warningf("Runner[%v] ActiveReader %s was closed", ar.runnerName, ar.originpath)
	err := ar.br.Close()
	if atomic.CompareAndSwapInt32(&ar.status, reader.StatusRunning, reader.StatusStopping) {
		log.Warningf("Runner[%v] ActiveReader %s was closing", ar.runnerName, ar.originpath)
	} else {
		return err
	}
...
func NewReader(meta *reader.Meta, conf conf.MapConf) (reader.Reader, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			log.Infof("Runner[%v] %v recover from meta error %v, ignore...", meta.RunnerName, logPathPattern, err)
		} else {
			log.Warningf("Runner[%v] %v recover from meta error %v, ignore...", meta.RunnerName, logPathPattern, err)
		}
		bufsize = 0
		err = nil
	}
	...
	if bufsize > 0 {
		if _, err = meta.ReadBuf(buf); err != nil {
			if os.IsNotExist(err) {
				log.Infof("Runner[%v] read buf file %v error %v, ignore...", meta.RunnerName, meta.BufFile(), err)
			} else {
				log.Warningf("Runner[%v] read buf file %v error %v, ignore...", meta.RunnerName, meta.BufFile(), err)
			}
		} else {
			err = jsoniter.Unmarshal(buf, &cacheMap)
			if err != nil {
				log.Warningf("Runner[%v] Unmarshal read buf cache error %v, ignore...", meta.RunnerName, err)
			}
		}
		err = nil
	}
...
func (r *Reader) statLogPath() {
	...
	if len(r.fileReaders) >= r.maxOpenFiles {
		log.Warningf("Runner[%v] %v meet maxOpenFiles limit %v, ignore Stat new log...", r.meta.RunnerName, r.Name(), r.maxOpenFiles)
		return
	}
	...
	if len(matches) > 0 {
		log.Infof("Runner[%v] statLogPath %v find matches: %v", r.meta.RunnerName, r.logPathPattern, strings.Join(matches, ", "))
	}
	...
	for _, mc := range matches {
		...
		if fi.IsDir() {
			log.Infof("Runner[%v] %v is dir, mode[tailx] only support read file, ignore this match...", r.meta.RunnerName, mc)
			continue
		}
		r.armapmux.Lock()
		...
		r.armapmux.Unlock()
		if ok {
			log.Infof("Runner[%v] <%v> is collecting, ignore...", r.meta.RunnerName, rp)
			continue
		}
		r.armapmux.Lock()
		...
		if cacheline == "" &&
			...
			log.Infof("Runner[%v] <%v> is expired, ignore...", r.meta.RunnerName, mc)
			continue
		}
		...
		if atomic.LoadInt32(&r.status) != reader.StatusStopped {
			...
			log.Warningf("Runner[%v] %v NewActiveReader but reader was stopped, ignore this...", r.meta.RunnerName, mc)
		}
		r.armapmux.Unlock()
		if atomic.LoadInt32(&r.status) != reader.StatusStopped {
			go ar.Run()
		} else {
			log.Warningf("Runner[%v] %v NewActiveReader but reader was stopped, will not running...", r.meta.RunnerName, mc)
		}
	}
