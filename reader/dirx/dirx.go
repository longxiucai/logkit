...
func NewReader(meta *reader.Meta, conf conf.MapConf) (reader.Reader, error) {
	...
	if bufsize > 0 {
		if _, err = meta.ReadBuf(buf); err != nil {
			if os.IsNotExist(err) {
				log.Infof("Runner[%v] read buf file[%v] failed: %v, ignore...", meta.RunnerName, meta.BufFile(), err)
			} else {
				log.Warningf("Runner[%v] read buf file[%v] failed: %v, ignore...", meta.RunnerName, meta.BufFile(), err)
			}
		} else {
			err = jsoniter.Unmarshal(buf, &cachedLines)
			if err != nil {
				log.Warningf("Runner[%v] unmarshal read buf cache failed: %v, ignore...", meta.RunnerName, err)
			}
		}
	}
...
func (r *Reader) statLogPath() {
	...
	if r.dirReaders.Num() >= r.maxOpenFiles {
		log.Warningf("Runner[%v] has met 'maxOpenFiles' limit %v, stat new log ignored", r.meta.RunnerName, r.maxOpenFiles)
		return
	}
	...
	if len(matches) == 0 {
		log.Infof("Runner[%v] no match found after stated log path %q", r.meta.RunnerName, r.logPathPattern)
		return
	}
	log.Infof("Runner[%v] %d matches found after stated log path %q: %v", r.meta.RunnerName, len(matches), r.logPathPattern, matches)

	var newPaths []string
	for _, m := range matches {
		logPath, fi, err := GetRealPath(m)
		if err != nil {
			log.Warningf("Runner[%v] file pattern %v match %v stat failed: %v, ignored this match", r.meta.RunnerName, r.logPathPattern, m, err)
			continue
		}
		if !fi.IsDir() {
			log.Infof("Runner[%v] %v is a file, mode[dirx] only supports stat directory, ignored this match", r.meta.RunnerName, m)
			continue
		}
		if r.dirReaders.HasReader(logPath) {
			log.Infof("Runner[%v] %q is collecting, ignored this path", r.meta.RunnerName, logPath)
			continue
		}

		// 过期的文件不追踪，除非之前追踪的并且有日志没读完
		if !r.dirReaders.hasCachedLine(logPath) && HasDirExpired(logPath, r.dirReaders.expire) {
			log.Infof("Runner[%v] log path %q has expired, ignored this time", r.meta.RunnerName, logPath)
			continue
		}
		...
		if r.hasStopped() {
			log.Warningf("Runner[%v] created new reader for log path %q but daemon reader has stopped, will not run at this time", r.meta.RunnerName, logPath)
			continue
		}
	...
	if len(newPaths) > 0 {
		...
		log.Infof("Runner[%v] stat log path has not found any new log path", r.meta.RunnerName)
	}
}
