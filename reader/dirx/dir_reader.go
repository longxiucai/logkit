...
func (dr *dirReader) Run() {
	...
	for {
		if atomic.LoadInt32(&dr.status) == reader.StatusStopping || atomic.LoadInt32(&dr.status) == reader.StatusStopped {
			...
			log.Warningf("Runner[%v] log path[%v] reader has stopped", dr.runnerName, dr.originalPath)
			return
		}
		...
		if len(dr.readcache) == 0 {
			...
			if err != nil && err != io.EOF && err != os.ErrClosed {
				log.Warningf("Runner[%v] log path[%v] reader read failed: %v", dr.runnerName, dr.originalPath, err)
			...
			if len(dr.readcache) == 0 {
				...
				if err == io.EOF {
					...
					log.Infof("Runner[%v] log path[%v] reader met EOF and becomes inactive now, sleep 5 seconds", dr.runnerName, dr.originalPath)
		...
		log.Infof("Runner[%v] %v >>>>>> read cache[%v] line cache [%v]", dr.runnerName, dr.originalPath, dr.readcache, string(dr.br.FormMutiLine()))
		repeat := 0
		for {
			if len(dr.readcache) == 0 {
				break
			...
			if atomic.LoadInt32(&dr.status) == reader.StatusStopping || atomic.LoadInt32(&dr.status) == reader.StatusStopped {
				log.Infof("Runner[%v] log path[%v] reader has stopped when waits to send data", dr.runnerName, dr.originalPath)
				atomic.CompareAndSwapInt32(&dr.status, reader.StatusStopping, reader.StatusStopped)
				return
			}
...
func (dr *dirReader) Close() error {
	defer log.Warningf("Runner[%v] log path[%v] reader has closed", dr.runnerName, dr.originalPath)
	err := dr.br.Close()
	if atomic.CompareAndSwapInt32(&dr.status, reader.StatusRunning, reader.StatusStopping) {
		log.Warningf("Runner[%v] log path[%v] reader is closing", dr.runnerName, dr.originalPath)
	} else {
		return err
	}
...
func (drs *dirReaders) checkExpiredDirs() {
	...
	if len(expiredDirs) > 0 {
		...
		log.Infof("Runner[%v] no log path has expired", drs.meta.RunnerName)
	}
}
