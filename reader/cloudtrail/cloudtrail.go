...
DONE:
	for s3file := range sourceFiles {
		select {
		case <-s.quitChan:
			log.Warningf("Runner[%v] daemon has stopped, task is interrupted", s.meta.RunnerName)
			break DONE
		default:
		}
		...
		if !s.syncedFiles[basename] {
			...
			log.Infof("Runner[%v] start syncing: s3://%s/%s -> %s", s.meta.RunnerName, bucket.Name, s3file, filePath)
			...
			go func(filePath string, bucket *s3.Bucket, s3file string) {
				...
				syncedChan <- s3file
				log.Infof("Runner[%v] sync completed: s3://%s/%s -> %s", s.meta.RunnerName, bucket.Name, s3file, filePath)
				pool <- struct{}{}
			}(filePath, bucket, s3file)
		} else {
			log.Infof("Runner[%v] %q already synced, skipped this time", s.meta.RunnerName, unzipPath)
		}
	}
	wg.Wait()
}
