...
func NewMetaWithConf(conf conf.MapConf) (meta *Meta, err error) {
	...
	if metapath == "" {
		...
		log.Infof("Runner[%v] Using %s as default metaPath", runnerName, metapath)
	}
	...
	if err != nil {
		log.Warningf("Runner[%v] %s - newMeta failed, err:%v", runnerName, metapath, err)
		return
	}
...
func (m *Meta) CleanExpiredSubMetas(expire time.Duration) {
	...
	log.Infof("Meta %q has finished cleaning submetas", m.Dir)
}
...
func (m *Meta) ReadOffset() (currFile string, offset int64, err error) {
	...
	if err != nil {
		log.Infof("meta file format err %v", err)
		return
	}
...
func (m *Meta) getDoneFiles() (doneFiles []File, err error) {
	...
	for _, f := range files {
		if f.IsDir() {
			log.Infof("Runner[%v] search file done skipped dir %v", m.RunnerName, f.Name())
			continue
		}
		fname := f.Name()
