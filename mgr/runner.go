...
import (
	...
	_ "github.com/longxiucai/logkit/parser/builtin"
	"github.com/longxiucai/logkit/parser/qiniu"
	"github.com/longxiucai/logkit/reader"
	_ "github.com/longxiucai/logkit/reader/builtin"
	"github.com/longxiucai/logkit/reader/cloudtrail"
	"github.com/longxiucai/logkit/router"
	"github.com/longxiucai/logkit/sender"
	_ "github.com/longxiucai/logkit/sender/builtin"
	"github.com/longxiucai/logkit/transforms"
	"github.com/longxiucai/logkit/transforms/ip"
...
func NewLogExportRunnerWithService(info RunnerInfo, reader reader.Reader, cleaner *cleaner.Cleaner, parser parser.Parser,
	...
	runner.meta = meta
	if cleaner == nil {
		log.Warningf("%v's cleaner was disabled", info.RunnerName)
	}
...
func NewLogExportRunner(rc RunnerConfig, cleanChan chan<- cleaner.CleanSignal, rr *reader.Registry, pr *parser.Registry, sr *sender.Registry) (runner *LogExportRunner, err error) {
	...
	if rc.ParserConf == nil {
		log.Warning(rc.RunnerName + " parser conf is nil, use raw parser as default")
...
func (r *LogExportRunner) readDatas(dr reader.DataReader, dataSourceTag string) []Data {
	...
	for !r.batchFullOrTimeout() {
		...
		if len(data) <= 0 {
			log.Infof("Runner[%v] data reader %s got empty data", r.Name(), r.reader.Name())
			continue
		}
...
func (r *LogExportRunner) readLines(dataSourceTag string) []Data {
	...
	for !r.batchFullOrTimeout() {
		...
		if len(line) <= 0 {
			log.Infof("Runner[%v] reader %s no more content fetched sleep 1 second...", r.Name(), r.reader.Name())
	...
	if len(lines) <= 0 {
		log.Infof("Runner[%v] fetched 0 lines", r.Name())
	...
	if dataSourceTag != "" {
		...
		if len(datas) <= len(froms) {
			...
			log.Infof("Runner[%v] datasourcetag add error, datas %v datasourceSkipIndex froms %v", datas, se.DatasourceSkipIndex, froms)
		}
	}
	return datas
}
...
func (r *LogExportRunner) Run() {
	...
	for {
		if atomic.LoadInt32(&r.stopped) > 0 {
			log.Infof("Runner[%v] exited from run", r.Name())
		...
		// send data
		if len(datas) <= 0 {
			log.Infof("Runner[%v] received parsed data length = 0", r.Name())
			continue
		}
		...
		log.Infof("Runner[%v] reader %s start to send at: %v", r.Name(), r.reader.Name(), time.Now().Format(time.RFC3339))
		success := true
		...
		log.Infof("Runner[%v] send %s finish to send at: %v", r.Name(), r.reader.Name(), time.Now().Format(time.RFC3339))
	}
}
...
func addSourceToData(sourceFroms []string, se *StatsError, datas []Data, datasourceTagName, runnerName string) []Data {
	...
	for i, v := range sourceFroms {
		...
		if dt, ok := datas[j][datasourceTagName]; ok {
			log.Infof("Runner[%v] datasource tag already has data %v, ignore %v", runnerName, dt, v)
		} else {
...
func (r *LogExportRunner) Stop() {
	...
	if err != nil {
		...
		log.Warningf("Runner[%v] reader %v of runner %v closed", r.Name(), r.reader.Name(), r.Name())
	}
	...
	select {
	case <-r.exitChan:
		log.Warningf("runner %v has been stopped", r.Name())
	case <-timer.C:
	...
	for _, t := range r.transformers {
		if c, ok := t.(io.Closer); ok {
			if err := c.Close(); err != nil {
				log.Warningf("Close transform failed, %v", err)
			}
		}
	}
	...
	for _, s := range r.senders {
		...
		if err != nil {
			...
			log.Warningf("Runner[%v] sender %v closed", r.Name(), s.Name())
		}
	}
...
func (r *LogExportRunner) batchFullOrTimeout() bool {
	...
	if r.MaxBatchLen > 0 && int(r.batchLen) >= r.MaxBatchLen {
		log.Infof("Runner[%v] meet the max batch length %v", r.RunnerName, r.MaxBatchLen)
		return true
	}
	// 达到最大字节数
	if r.MaxBatchSize > 0 && int(r.batchSize) >= r.MaxBatchSize {
		log.Infof("Runner[%v] meet the max batch size %v", r.RunnerName, r.MaxBatchSize)
		return true
	}
	// 超过最长的发送间隔
	if time.Now().Sub(r.lastSend).Seconds() >= float64(r.MaxBatchInterval) {
		log.Infof("Runner[%v] meet the max batch send interval %v", r.RunnerName, r.MaxBatchInterval)
		return true
	}
	// 如果任务已经停止
	if atomic.LoadInt32(&r.stopped) > 0 {
		log.Warningf("Runner[%v] meet the stopped signal", r.RunnerName)
		return true
	}
	return false
}
...
func (r *LogExportRunner) StatusRestore() {
	...
	if err != nil {
		log.Warningf("Runner[%v] restore status failed: %v", r.RunnerName, err)
		return
	}
...
func (r *LogExportRunner) StatusBackup() {
	...
	if err != nil {
		log.Warningf("runner %v, backup status failed", r.RunnerName)
	} else {
...
func MergeEnvTags(name string, tags map[string]interface{}) map[string]interface{} {
	...
	if value := os.Getenv(name); value != "" {
		...
		if err != nil {
			log.Warningf("get env tags error: %v", err)
			return tags
		}
	}
