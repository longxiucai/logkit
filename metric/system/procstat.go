...
func (p *Procstat) findPids() ([]PID, error) {
	...
	if p.PidFile != "" {
		if ps, err = f.PidFile(p.PidFile); err != nil {
			log.Warningf("get pidfile %v error %v", p.PidFile, err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.Exe != "" {
		if ps, err = f.Pattern(p.Exe); err != nil {
			log.Warningf("get pids by exec 'pgrep %v' error %v", p.Exe, err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.Pattern != "" {
		if ps, err = f.FullPattern(p.Pattern); err != nil {
			log.Warningf("get pids by exec 'pgrep -f %v error %v", p.Pattern, err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.User != "" {
		if ps, err = f.Uid(p.User); err != nil {
			log.Warningf("get pids by exec 'pgrep -u %v error %v", p.User, err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.SystemdUnit != "" {
		if ps, err = p.systemdUnitPIDs(); err != nil {
			log.Warningf("get pids by exec 'systemctl show %v error %v", p.SystemdUnit, err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.CGroup != "" {
		if ps, err = p.cgroupPIDs(); err != nil {
			log.Warningf("get pids by cgroup error %v", p.CGroup, err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.CpuTop10 {
		if ps, err = p.PCpuTop10(); err != nil {
			log.Warningf("get pids of cpu usage top 10 error %v", err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.MemTop10 {
		if ps, err = p.PMemTop10(); err != nil {
			log.Warningf("get pids of mem usage top 10 error %v", err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.Supervisord {
		if ps, err = p.childProcess("supervisord"); err != nil {
			log.Warningf("get pids of supervisord managed process error %v", err)
		} else {
			pids = append(pids, ps...)
		}
	}
	if p.DaemonTools {
		if ps, err = p.childProcess("daemontools"); err != nil {
			log.Warningf("get pids of daemontools managed process error %v", err)
		} else {
			pids = append(pids, ps...)
		}
	}
...
func (p *Procstat) PCpuTop10() (pids []PID, err error) {
	...
	if p.kernel == GoOSMac {
		...
		log.Warningf("not support kernel %v, ignored it", p.kernel)
		return
	}
