...
func getNumDisk() (mountsNum int) {
	...
	if err != nil {
		log.Infof("can't find /sbin/fdisk on your system PATH, will not collect")
		return 0
	}
...
func getNumService() int {
	...
	if err != nil {
		log.Infof("can't find supervisorctl on your system, will not collect service number")
		return 0
	}
	out, err = exec.Command("supervisorctl", "status").Output()
	if err != nil {
		log.Infof("get service number from supervisor error %v", err)
		return 0
	}
	//去掉首行
