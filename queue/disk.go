...
func NewDiskQueue(opts NewDiskQueueOptions) BackendQueue {
	...
	if err != nil {
		log.Warningf("ERROR: diskqueue(%s) failed to retrieveMetaData - %s", d.name, err)
	}
...
func (d *diskQueue) exit(deleted bool) error {
	...
	if deleted {
		log.Warningf("DISKQUEUE(%s): deleting", d.name)
	} else {
		log.Warningf("DISKQUEUE(%s): closing", d.name)
	}
...
func (d *diskQueue) Empty() error {
	...
	log.Warningf("DISKQUEUE(%s): emptying", d.name)
...
func (d *diskQueue) deleteAllFiles() error {
	...
	if innerErr != nil && !os.IsNotExist(innerErr) {
		log.Warningf("ERROR: diskqueue(%s) failed to remove metadata file - %s", d.name, innerErr)
		return innerErr
	}
...
func (d *diskQueue) skipToNextRWFile() error {
	...
	for i := d.readFileNum; i <= d.writeFileNum; i++ {
		...
		if innerErr != nil && !os.IsNotExist(innerErr) {
			log.Warningf("ERROR: diskqueue(%s) failed to remove data file - %s", d.name, innerErr)
			err = innerErr
		}
	}
...
func (d *diskQueue) readOne() ([]byte, error) {
	...
	if d.readFile == nil {
		...
		log.Warningf("DISKQUEUE(%s): readOne() opened %s", d.name, curFileName)
...
func (d *diskQueue) writeOne(data []byte) error {
	...
	if d.writeFile == nil {
		...
		log.Warningf("DISKQUEUE(%s): writeOne() opened %s", d.name, curFileName)
	...
	if d.writePos > d.maxBytesPerFile {
		...
		err = d.sync()
		if err != nil {
			log.Warningf("ERROR: diskqueue(%s) failed to sync - %s", d.name, err)
		}
...
func (d *diskQueue) moveForward() {
	...
	if oldReadFileNum != d.nextReadFileNum {
		...
		if utils.IsExist(fpath) {
			...
			if err != nil {
				log.Warningf("ERROR: failed to stat file %q: %s", fpath, err)
			} else {
				atomic.AddInt64(&d.currentDiskUsedBytes, -fi.Size())
			}
			...
			err = os.Remove(fpath)
			if err != nil {
				log.Warningf("ERROR: failed to remove file %q: %s", fpath, err)
			}
		}
	}
...
DONE:
		...
		if origin == FromNone {
			if (d.readFileNum < d.writeFileNum) || (d.readPos < d.writePos) {
				if d.nextReadPos == d.readPos {
					...
					if err != nil {
						log.Warningf("ERROR: reading from diskqueue(%s) at %d of %s - %s",
	...
	syncTicker.Stop()
	log.Warningf("DISKQUEUE(%s): closing ... ioLoop", d.name)
	if d.enableMemoryQueue {
		d.saveToDisk()
	}
